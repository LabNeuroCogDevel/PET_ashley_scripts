...
MAXJOBS=5
WAITTIME=60
JOBCFGDIR=/Volumes/Hera/Projects/mMR_PETDA/scripts_ashley
...
#MAXCENSOR=$(((1200 - 720))) # total timepoints - 480(40% of 1200)
#AP modified 07/23/2019 to make a more lenient censor threshold. 
#now want thres to be %40 of time points as opposed to 20%
#Update: program won't do it - 3dTproject requires that the total number of retained time pts be higher than total number of fixed regressors. 

# use the folders given or run on all
[ $# -eq 0 ] && echo "USAGE: $0 all; $0 /Volumes/Phillips/mMR_PETDA/subjs/11434_20160601/" && exit 1
#[ $# -eq 0 ] && echo "USAGE: $0 all; $0 /Volumes/Hera/Projects/mMR_PETDA/subjs/11434_20160601/" && exit 1

if [ $1 == "all" ]; then
  echo "# finding all subjects"
  files=( /Volumes/Phillips/mMR_PETDA/subjs/1*_2*/func);
  # files=( /Volumes/Hera/Projects/mMR_PETDA/subjs/1*_2*/func);
...
project() {
   ...
   # data is in two places (as of 20190717)
   subj_root_old=${subj_root/Hera\/Projects/Phillips}
   subj_root_new=${subj_root/Phillips/Hera\/Projects}

   local func_dir=$subj_root_old/func/ # old function stuff on phillips
   ...
   this_dir=$subj_root_new/background_connectivity
   ...
   [ ! -r $nuisance ] && echo "need to make $nuisance" 
