...
cd $DATADIR
conditions="R_NAcc L_NAcc VTA R_Caudate L_Caudate R_Putamen L_Putamen" #run through each seed ROI 

for subj in $DATADIR/1*; do

   # 20190509 dont bother trying if we dont have/can't create the censor file
   ...
   for condition in $conditions; do
      ...
      ts_file=$subj/background_connectivity/*_${condition}_gsr.1D #find the seed ROI ts file
          
      echo $subj $sub_id $condition
      if [[ -r $ts_file || -r $subj/background_connectivity/cbgnfswdktm_tent_resid.nii.gz ]]; #find the GLM resid file (background)
      then
